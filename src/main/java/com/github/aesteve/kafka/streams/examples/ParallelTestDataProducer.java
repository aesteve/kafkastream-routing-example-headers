...
public class ParallelTestDataProducer {
    ...
    public static final int NB_THREADS = 20;
        ...
        var tasks = chunked(tenantIds, chunkSize)
                ...
                .map(tenantIdChunk ->
                        (Callable<TestOutcome>)
                                () -> produceTestMessages(TOTAL_RECORDS_TO_SEND, BULK_SIZE, PAYLOAD_SIZE, tenantIdChunk, props)
                )
                .toList();
        executorService.invokeAll(tasks).stream().map(fut -> {
            try {
                return fut.get();
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }).forEach(res -> LOG.info(res.toString()));
    }
